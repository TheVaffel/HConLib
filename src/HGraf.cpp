    ...
    DelaunayTriangles delaunay_triangulation(const std::vector<falg::Vec2>& in_points,
        ...
        for(uint32_t i = 0; i < in_points.size(); i++) {
            ...
            for(uint32_t j = i + 1; j < in_points.size(); j++) {
                if((in_points[i] - in_points[j]).sqNorm() < boundary_w * boundary_h * 2e-3) {
                    ok = false;
                    break;
                } 
            }
            ...
            if(ok) {
                ...
                std::cerr << "Discarded point " << i << " because it was too close to another point" << std::endl;
            }
        }
        // del_tri.points = in_points;
        ...
        // Remove bounding points added at the beginning
        /* for(std::list<TriInd>::iterator it = triangulation.begin(); it != triangulation.end(); it++) {
            TriInd pr = *it;

            for(int i = 0; i < 3; i++) {
                if(pr.inds[i] >= points.size() - 4) {
                    std::list<TriInd>::iterator thisit = it;
                    thisit++;
