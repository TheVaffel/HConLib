...
VkPipeline winvulk_init_pipeline(winvulk_vulkan_state*);
...
struct WinvulkBuffer{
  VkBuffer buffer;
  VkDeviceMemory memory;
};

WinvulkBuffer createBuffer(winvulk_vulkan_state*, uint usage, uint size);
VkResult setBuffer(winvulk_vulkan_state*,const WinvulkBuffer&, const void * data, uint size);

VkPipeline createPipeline(winvulk_vulkan_state*);
...
class ColorModel{
  ...
  WinvulkBuffer vertexBuffer;

  WinvulkBuffer colorBuffer;

  WinvulkBuffer indexBuffer;
...
uint winvulk_get_memory_type_index(winvulk_vulkan_state* vs, uint type_bits, VkFlags requirements_mask){
  for(uint i = 0; i < vs->device_memory_props.memoryTypeCount; i++){
    if((type_bits & 1) == 1){
...
VkResult winvulk_find_device(winvulk_vulkan_state* vs){
  ...
  for(uint i = 0; i < device_count; i++){
    ...
    for(uint j = 0; j < vs->queue_family_count; j++){
...
VkResult winvulk_init_device(winvulk_vulkan_state* vs){
  ...
  for(uint i = 0 ; i < vs->queue_family_count; i++){
...
VkResult winvulk_init_swapchain(winvulk_vulkan_state* vs){
  ...
  for(uint i = 0; i < vs->swapchain_image_count; i++){
...
VkResult winvulk_init_vertex_buffer(winvulk_vulkan_state* vs){
  ...
  WinvulkBuffer buf = createBuffer(vs, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, sizeof(g_vb_solid_face_colors_Data));
  setBuffer(vs, buf, g_vb_solid_face_colors_Data, sizeof(g_vb_solid_face_colors_Data));
  vs->vertex_buffer = buf.buffer;
...
VkPipeline winvulk_init_pipeline(winvulk_vulkan_state* vs){
...
void updateMVP(const Matrix4& nmvp, winvulk_vulkan_state* vs){
  unsigned char *pData;
  
  VkMemoryRequirements mem_reqs;
  vkGetBufferMemoryRequirements(vs->device, vs->uniform_buffer, &mem_reqs);
  
  VkResult res = vkMapMemory(vs->device, vs->uniform_memory, 0, mem_reqs.size, 0, (void**)&pData);
  if(res != VK_SUCCESS){
    printf("Could not map memory\n");
    exit(0);
  }
...
WinvulkBuffer createBuffer(winvulk_vulkan_state* vs, uint usage, uint size){
  WinvulkBuffer wBuffer;
  ...
  buf_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
  buf_info.flags = 0;
  VkResult res = vkCreateBuffer(vs->device, &buf_info, NULL, &wBuffer.buffer);
  if(res != VK_SUCCESS){
  ...
  VkMemoryRequirements mem_reqs;
  vkGetBufferMemoryRequirements(vs->device, wBuffer.buffer, &mem_reqs);
  ...
  alloc_info.allocationSize = mem_reqs.size;
  alloc_info.memoryTypeIndex = winvulk_get_memory_type_index(vs, mem_reqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT); //Ensures reachability from host
  ...
  res = vkAllocateMemory(vs->device, &alloc_info, NULL, &wBuffer.memory);
  if(res != VK_SUCCESS){
    printf("Could not allocate memory\n");
    exit(0);
  }

  res = vkBindBufferMemory(vs->device, wBuffer.buffer, wBuffer.memory, 0);
  if(res != VK_SUCCESS){
    printf("Could not bind buffer and memory\n");
  }
...
VkResult setBuffer(winvulk_vulkan_state* vs, const WinvulkBuffer& buffer, const void* data, uint size){
  void * map;
  VkResult res;

  res = vkMapMemory(vs->device, buffer.memory, 0, VK_WHOLE_SIZE, 0, &map);
  if(res != VK_SUCCESS){
    printf("Could not map memory\n");
    exit(0);
  }
  ...
  vkUnmapMemory(vs->device, buffer.memory);

  return res;
}

VkPipeline createPipeline(winvulk_vulkan_state* vs){
  ...
  pipelineCache.pInitialData = NULL;
  pipelineCache.flags = 0;
  VkResult res = vkCreatePipelineCache(vs->device, &pipelineCache, NULL, &r_pipelineCache);
  if(res != VK_SUCCESS){
    printf("Could not create pipeline cache\n");
  }
  ...
  res = vkCreateGraphicsPipelines(vs->device, r_pipelineCache, 1, &pipeline, NULL, &r_pipeline);
  if(res != VK_SUCCESS){
    printf("Could not create pipeline\n");
    exit(0);
  }
...
void winvulk_render(winvulk_vulkan_state* vs, VkPipeline pipeline, const WinvulkBuffer& vertexBuffer, const WinvulkBuffer& indexBuffer){
  ...
  VkRect2D screenRect;
  screenRect.extent.width = vs->width;
  screenRect.extent.height = vs->height;
  ...
  vkResetCommandBuffer(vs->cmd_buffer, VK_COMMAND_BUFFER_RESET_RELEASE_RESOURCES_BIT);
  VkResult res = vkBeginCommandBuffer(vs->cmd_buffer, &cmd_buf_info);
  if(res != VK_SUCCESS){
  ...
  res = vkCreateSemaphore(vs->device, &imageAcquiredSemaphoreCreateInfo, NULL, &imageAcquiredSemaphore);
  if(res != VK_SUCCESS){
  ...
  vkCmdBindPipeline(vs->cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipeline);
  ...
  vkCmdBindVertexBuffers(vs->cmd_buffer, 0,
			 ...
			 &vertexBuffer.buffer,
			 offsets);
  vkCmdBindIndexBuffer(vs->cmd_buffer, indexBuffer.buffer, 0, VK_INDEX_TYPE_UINT32);

  vkCmdClearAttachments(vs->cmd_buffer,
  			2,
  			clearAttachments,
  			1,
  			&clearRect);
  ...
  //vkCmdDraw(vs->cmd_buffer, 18, 1, 0, 0);
  vkCmdDrawIndexed(vs->cmd_buffer, 2*3, 1, 0, 0, 0);
...
void winvulk_render_loop(winvulk_vulkan_state* vs){
  ...
  //WinvulkBuffer vertexBuffer = createBuffer(vs, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, 3*4*sizeof(float));

  //setBuffer(vs, vertexBuffer, test_vertices, 3*4*sizeof(float));

  //WinvulkBuffer colorBuffer = createBuffer(vs, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, 3*4*sizeof(float));

  //setBuffer(vs, colorBuffer, test_colors, 3*4*sizeof(float));

  WinvulkBuffer indexBuffer = createBuffer(vs, VK_BUFFER_USAGE_INDEX_BUFFER_BIT, 3*4*sizeof(float));

  setBuffer(vs, indexBuffer, test_indices, 2*3*sizeof(uint32_t));

  WinvulkBuffer allBuffer = createBuffer(vs, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, 3*2*4*sizeof(float));

  setBuffer(vs, allBuffer, test_all, 3*2*4*sizeof(float));

  

  VkPipeline pipeline = winvulk_init_pipeline(vs);//createPipeline(vs);
  ...
  while(1){
    ...
    Matrix4 usableMvp = ~mvp;
    updateMVP(usableMvp, vs);
    winvulk_render(vs, pipeline, allBuffer, indexBuffer);
    count++;
